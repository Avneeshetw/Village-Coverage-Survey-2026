...
# ----------------- GOOGLE SHEET CONNECTION (ZERO-PEM ERROR FIX) -----------------
def get_gspread_client():
    ...
    # Strictly handle line breaks without breaking string characters
    pk = str(creds_dict.get("private_key", ""))
    pk = pk.replace("\\n", "\n").replace('\r', '').strip()
    
    # Ensure quotes wrapper is clean
...
df_f5 = df_f4[df_f4['Distributor Name, Town DRB Code'] == selected_dist] if selected_dist != "Select..." and not df_f5.empty else pd.DataFrame(columns=df_master.columns)
